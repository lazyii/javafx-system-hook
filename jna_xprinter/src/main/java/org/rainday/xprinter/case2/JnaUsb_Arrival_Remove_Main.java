...
/**
 * usb设备连接、移除（Arrival/Remove） 时进行提醒
 * <p>
 * 在 Windows 操作系统里，当窗口显示之后，它就可以接收到系统源源不断地发过来的消息， 然后窗口就需要处理这些消息，因此就需要一个函数来处理这些消息。 在 API 里定义了一个函数为回调函数，当系统需要向窗口发送消息时，就会调用窗口给出的回调函数 WindowProc ， 如果
 * WindowProc 函数不处理这个消息，就可以把它转向 DefWindowProc 函数来处理，这是系统的默认消息处理函数。
 *
 * @author admin
 * @version 1.0 edit by admin at 2021-06-26 20:15:42
 * @date 2021-06-26 20:15:42
 */
public class JnaUsb_Arrival_Remove_Main implements WindowProc {

    /**
     * Instantiates a new win32 window test.
     */
    public JnaUsb_Arrival_Remove_Main() {
        ...
        wClass.lpfnWndProc = JnaUsb_Arrival_Remove_Main.this;
    ...
    public static void main(String[] args) {
        new JnaUsb_Arrival_Remove_Main();
    }
}
